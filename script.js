try {
    ...
    function playVid(originalBlob) {
        ...
        function openFullScreen() { fscreen = 'yes'; app.style = 'top: 0px; bottom:0px;z-index: 50;display: flex;justify-content: center;align-items: center;'; var isLandscape = screen.width > screen.height || window.innerWidth > window.innerHeight; if (isLandscape) { rotate = 0; scale = 1; vp.style = 'width:100%;height:100%;transform: scale(' + scale + ') rotate(' + rotate + 'deg);z-index: 998'; } else { rotate = 90; scale = 1.36; vp.style = 'transform: scale(' + scale + ') rotate(' + rotate + 'deg);z-index: 998'; } document.querySelectorAll('#details')[0].style = 'display: none;'; if (subon === true) { if (rotate === 90) { subtitleBar.style = 'background: rgba(0,0,0,1);padding: 0px;position: absolute;top: auto;width: 100%;left: -99px;color: white;text-align: center;transform: rotate(90deg);display:block;z-index: 999'; } else { subtitleBar.style = 'background: rgba(0,0,0,1);padding: 0px;position: absolute;bottom: 20px;width: 100%;left: 0px;color: white;text-align: center;display:block;z-index: 999'; } } else { subtitleBar.style = 'display: none'; } }
